	...
	include('config.php');
		...
		*/
		if(!isset($_SESSION['id']) || !isset($_SESSION['pseudo'])){
			...
			$id = $_SESSION['id'];
			$sql = mysqli_query($cnx, "SELECT * FROM users WHERE id = '$id'") or die('Erreur de la requête SQL');
			...
			<br />
			 <?php
				...
				if(isset($_POST['submit'])){
					...
					$sql = mysqli_query($cnx, "SELECT * FROM users WHERE pseudo = '$pseudo' AND id != '".$donnees['id']."'") or die('Erreur de la requête SQL');
					...
					if($erreur == null){
						...
						*/							
						...
						$sql = mysqli_query($cnx, "UPDATE users SET pseudo = '$pseudo', nom = '$nom', prenom = '$prenom', sexe = '$sexe', password = '$password' WHERE id = '".$_SESSION['id']."'") or die('Erreur de la requête SQL');
						if($sql){
